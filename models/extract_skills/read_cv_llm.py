...
def extract_personal_info_with_llm(cv_text: str) -> Dict:
    """
    Extrahiert persönliche Informationen aus dem CV mit LLM.
    """
    if not HAS_REQUESTS:
        print("[WARNUNG] LLM nicht verfügbar für persönliche Daten")
        return extract_personal_info_regex(cv_text)
    
    print("Extrahiere persönliche Informationen mit LLM...")
    
    # Verwende mehr Text für bessere Ergebnisse
    text_preview = cv_text[:3000]
    
    prompt = f"""Analysiere den folgenden Lebenslauf und extrahiere die persönlichen Daten als JSON.
...
Wenn eine Information nicht vorhanden ist, verwende "N/A".
...
JSON:"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  LLM-Antwort: {answer[:200]}...")
            ...
            if json_match:
                try:
                    data = json.loads(json_match.group())
                    print(f"  Persönliche Daten extrahiert: {data.get('Vorname', 'N/A')} {data.get('Nachname', 'N/A')}")
                    return data
                except json.JSONDecodeError as e:
                    print(f"[WARNUNG] JSON-Parsing fehlgeschlagen: {e}")
                    print(f"  JSON-String: {json_match.group()[:200]}")
        else:
            print(f"[WARNUNG] LLM-Request fehlgeschlagen: Status {response.status_code}")
    except Exception as e:
        print(f"[WARNUNG] Persönliche Daten-Extraktion fehlgeschlagen: {e}")
    
    # Fallback: Verwende Regex-basierte Extraktion
    print("[INFO] Verwende Regex-Fallback für persönliche Daten")
    return extract_personal_info_regex(cv_text)


def extract_personal_info_regex(cv_text: str) -> Dict:
    """
    Extrahiert persönliche Informationen mit Regex als Fallback.
    Unterstützt deutsche und englische CVs.
    """
    lines = cv_text.split('\n')
    ...
    first_lines = ' '.join(lines[:50])
    
    # Name-Extraktion: Häufig in den ersten Zeilen, oft groß geschrieben
    # Unterstützt deutsche Umlaute und englische Namen
    name_patterns = [
        r'([A-ZÄÖÜ][a-zäöüßA-Z]+)\s+([A-ZÄÖÜ][a-zäöüßA-Z]+)',  # Vorname Nachname (auch mit Großbuchstaben)
        r'Name[:\s]+([A-ZÄÖÜ][a-zäöüßA-Z]+)\s+([A-ZÄÖÜ][a-zäöüßA-Z]+)',
        r'^([A-Z][a-z]+)\s+([A-Z][a-z]+)',  # Englische Namen am Zeilenanfang
    ]
    
    for pattern in name_patterns:
        match = re.search(pattern, first_lines, re.MULTILINE)
        if match:
            result['Vorname'] = match.group(1)
            result['Nachname'] = match.group(2)
            print(f"  Regex: Name gefunden: {result['Vorname']} {result['Nachname']}")
            break
    
    # Beruf-Extraktion: Deutsch und Englisch
    beruf_patterns = [
        # Englische Berufsbezeichnungen
        r'(Chief\s+\w+\s+Officer|Senior\s+\w+|Junior\s+\w+|Lead\s+\w+)',
        r'(Software\s+Engineer|Data\s+Scientist|Product\s+Manager|Project\s+Manager|Account\s+Manager)',
        r'(Finance\s+Manager|Financial\s+Analyst|Business\s+Analyst|Marketing\s+Manager)',
        r'(Engineer|Developer|Manager|Analyst|Consultant|Designer|Architect|Specialist)',
        r'(Director|Coordinator|Assistant|Administrator|Technician|Officer|Executive)',
        r'(Accountant|Auditor|Controller|Treasurer|Bookkeeper)',
        # Deutsche Berufsbezeichnungen
        r'(Geschäftsführer|Abteilungsleiter|Teamleiter|Projektleiter)',
        r'(Ingenieur|Entwickler|Berater|Architekt|Spezialist)',
        r'Beruf[:\s]+([A-ZÄÖÜa-zäöüß\s]+)',
        r'Position[:\s]+([A-ZÄÖÜa-zäöüß\s]+)',
        r'Title[:\s]+([A-Za-z\s]+)',
        r'Role[:\s]+([A-Za-z\s]+)',
    ]
    
    for pattern in beruf_patterns:
        match = re.search(pattern, first_lines, re.IGNORECASE)
        if match:
            result['Beruf'] = match.group(1).strip()
            print(f"  Regex: Beruf gefunden: {result['Beruf']}")
            break
    
    # Adresse-Extraktion: Deutsche und englische Formate
    adresse_patterns = [
        # Deutsche Adressen: Musterstraße 123, 12345 Musterstadt
        r'([A-ZÄÖÜ][a-zäöüß]+(?:straße|str\.|strasse)[\s\d]+[,\s]*\d{4,5}[\s]+[A-ZÄÖÜ][a-zäöüß]+)',
        # Englische/internationale Adressen: 123 Main Street, City
        r'(\d+\s+[A-Z][a-z]+\s+(?:Street|St\.|Road|Rd\.|Avenue|Ave\.|Drive|Dr\.|Lane|Ln\.|Boulevard|Blvd\.)[,\s]+[A-Z][a-z]+)',
        # UK Format: City, Postcode
        r'([A-Z][a-z]+[,\s]+[A-Z]{1,2}\d{1,2}\s*\d[A-Z]{2})',
        # Einfaches Format: Stadt, Land
    ...
    for pattern in adresse_patterns:
        ...
        if match:
            ...
            print(f"  Regex: Adresse gefunden: {result['Adresse']}")
            break
...
def create_cv_excel(pdf_path: str, output_path: str, use_llm: bool = True):
    ...
    # Schritt 2: Extrahiere persönliche Daten
    personal_info = {}
    if use_llm and HAS_REQUESTS:
        personal_info = extract_personal_info_with_llm(cv_text)
    
    # Schritt 3: Verwende LLM zur Extraktion der Berufserfahrung
    if use_llm and HAS_REQUESTS:
    ...
    # Schritt 4: Validierung und Sortierung
    entries = validate_entries(entries)
    
    # Schritt 5: Export zu Excel mit persönlichen Daten
    df = pd.DataFrame(entries)
    
    # Füge persönliche Daten als separate Spalten hinzu
    if personal_info:
        df.insert(0, 'Vorname', personal_info.get('Vorname', 'N/A'))
        df.insert(1, 'Nachname', personal_info.get('Nachname', 'N/A'))
        df.insert(2, 'Beruf', personal_info.get('Beruf', 'N/A'))
        df.insert(3, 'Adresse', personal_info.get('Adresse', 'N/A'))
    ...
    # Schritt 6: Speichere Ergebnisse auch in txt-Datei im models-Ordner
    base_name = os.path.basename(pdf_path).replace('.pdf', '_ergebnisse.txt')
    txt_output_path = os.path.join('models', base_name)
    try:
        with open(txt_output_path, 'w', encoding='utf-8') as f:
            f.write("=" * 60 + "\n")
            f.write("CV-Extraktionsergebnisse\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"PDF-Datei: {pdf_path}\n")
            f.write(f"Excel-Ausgabe: {output_path}\n")
            f.write(f"Anzahl der Einträge: {len(entries)}\n")
            f.write(f"Extraktionsmethode: {'LLM (Ollama)' if use_llm and HAS_REQUESTS else 'Regex/Heuristiken'}\n")
            f.write(f"Datum: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            
            # Persönliche Daten (immer anzeigen)
            f.write("\n" + "=" * 60 + "\n")
            f.write("Persönliche Daten:\n")
            f.write("=" * 60 + "\n")
            f.write(f"Vorname: {personal_info.get('Vorname', 'N/A') if personal_info else 'N/A'}\n")
            f.write(f"Nachname: {personal_info.get('Nachname', 'N/A') if personal_info else 'N/A'}\n")
            f.write(f"Beruf: {personal_info.get('Beruf', 'N/A') if personal_info else 'N/A'}\n")
            f.write(f"Adresse: {personal_info.get('Adresse', 'N/A') if personal_info else 'N/A'}\n")
            
            f.write("\n" + "=" * 60 + "\n")
            f.write("Extrahierte Daten:\n")
            f.write("=" * 60 + "\n\n")
            f.write(df.to_string(index=False))
            f.write("\n\n" + "=" * 60 + "\n")
        print(f"\n[OK] Ergebnisse gespeichert in: {txt_output_path}")
    except Exception as e:
        print(f"\n[WARNUNG] Fehler beim Speichern der txt-Datei: {e}")
