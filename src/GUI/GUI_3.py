...
from GUI.Strukturierter_Datenimport_LLM import (
...
def get_smallest_free_candidate_id(conn):
...
def insert_candidate_record(payload, assign_min_free_id=False):
    ...
    try:
        ...
        if assign_min_free_id and "id" not in record and "id" in allowed:
            record["id"] = get_smallest_free_candidate_id(conn)
        if not record:
